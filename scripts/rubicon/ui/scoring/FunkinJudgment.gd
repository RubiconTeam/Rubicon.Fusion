class_name FunkinJudgment extends GDHitMaterialStatDisplay
...
func update_stats(_combo : int, _hit : int, _distance : float) -> void:
	...
	fade_tween.tween_property(judgment, "modulate", Color.TRANSPARENT, 0.2).set_delay(60.0 / Conductor.GetCurrentTimeChange().Bpm)
	fade_tween.play()
