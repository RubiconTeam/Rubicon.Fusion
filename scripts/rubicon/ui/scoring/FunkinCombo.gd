class_name FunkinCombo extends GDHitMaterialStatDisplay
...
func update_stats(_combo : int, _hit : int, _distance : float) -> void:
	...
	for i in split_digits.size():
		...
		fade_tween.tween_property(combo_spr, "modulate", Color.TRANSPARENT, 0.2).set_delay(60.0 / Conductor.GetCurrentTimeChange().Bpm * 2.0)
		fade_tween.play()
